      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-[var(--color-secondary)]">Pacient verificat</div>
          </div>
        </div>
...
export function GoogleReviewsSlider({ reviews, locale, rating, totalReviews, googleMapsUrl }: GoogleReviewsSliderProps) {
  ...
  const midPoint = Math.ceil(reviewsWithText.length / 2)
  const topRowReviews = reviewsWithText.slice(0, midPoint)
  const bottomRowReviews = reviewsWithText.slice(midPoint)
    ...
    <div className="space-y-10">
      ...
      {rating && totalReviews && (
        <div className="flex justify-center">
          <div className="inline-flex items-center gap-3 bg-[#faf6f1] border border-[#e8e0d5] rounded-full px-6 py-3">
            {/* Google G Icon */}
            <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
            ...
            {/* Rating Number */}
            <span className="text-base font-bold text-[#2a2118]">{rating}</span>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-4">
        <a
          className="group inline-flex items-center px-7 py-3.5 md:px-8 md:py-4 bg-[#2a2118] text-white rounded-full text-sm md:text-base font-semibold hover:shadow-[0_10px_40px_-10px_rgba(42,33,24,0.4)] transition-shadow duration-300 whitespace-nowrap"
          href="https://g.page/r/CRn1KUzHwfjFEBM/review"
          rel="noopener noreferrer"
          target="_blank"
        >
          <Star className="w-4 h-4 md:w-5 md:h-5 fill-current mr-2" />
          ...
          >
            <svg className="w-4 h-4 md:w-5 md:h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.25}>
        ...
        {googleMapsUrl && (
          <a
            className="group inline-flex items-center px-7 py-3.5 md:px-8 md:py-4 bg-white border border-[#e8e0d5] hover:border-[#d4c4b0] text-[#2a2118] rounded-full text-sm md:text-base font-semibold hover:shadow-[0_10px_40px_-10px_rgba(139,115,85,0.18)] transition-all duration-300 whitespace-nowrap"
            href={googleMapsUrl}
            rel="noopener noreferrer"
            target="_blank"
          >
            <svg className="w-4 h-4 md:w-5 md:h-5 flex-shrink-0 mr-2" fill="none" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
